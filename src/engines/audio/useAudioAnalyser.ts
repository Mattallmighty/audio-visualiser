...
export function useAudioAnalyser(config: AudioAnalyserConfig = EMPTY_CONFIG) {
  ...
  // Throttle state updates to prevent React infinite loop
  // Update state every 3 frames (~20fps) instead of 60fps
  const frameCounterRef = useRef<number>(0);
  const updateThrottle = 2;
  ...
  useEffect(() => {
    processAudioRef.current = () => {
      ...
      // Throttle state updates to prevent infinite render loop
      // Only update React state every N frames, not every frame
